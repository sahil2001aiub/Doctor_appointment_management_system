...
<section class="appointment-form-section">
    ...
    <form method="POST" action="" class="appointment-form">
        ...
        <button type="submit" name="add_assistant">Add Assistant</button>
    </form>
</section>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_assistant'])) {
    ...
    if (empty($errors)) {
        ...
        $stmt = $conn->prepare("INSERT INTO users (name, phone, email, password, user_type) VALUES (?, ?, ?, ?, 'assistant')");
        $stmt->bind_param("ssss", $name, $phone, $email, $hashed_password);

        if ($stmt->execute()) {
            echo "<p class='success-message'>Assistant added successfully!</p>";
        } else {
